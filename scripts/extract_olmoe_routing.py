...
def extract_olmoe_routing(
    ...
    Args:
        ...
        dataset: Name of the dataset(s) to use (must be registered in dataset_info.json).
                 Multiple datasets can be specified separated by commas, e.g., "dataset1,dataset2,dataset3"
    ...
    Usage:
        ...
        # Single GPU, multiple datasets
        python extract_olmoe_routing.py \
            --model_name_or_path allenai/OLMoE-1B-7B-0924 \
            --dataset alpaca_en_demo,alpaca_zh_demo,identity \
            --template default \
            --output_dir routing_outputs

        # Multi-GPU (4 GPUs), multiple datasets
    ...
    # Parse dataset names (support comma-separated list)
    ...
    for dataset_name in dataset_names:
        ...
        if not dataset_info[dataset_name].get("file_name"):
            raise ValueError(f"No file_name specified for dataset '{dataset_name}' in dataset_info.json")
    ...
    if accelerator.is_main_process:
        ...
        print(f"Datasets: {', '.join(dataset_names)} ({len(dataset_names)} total)")
    ...
    if accelerator.is_main_process:
        print("\n[Step 1] Loading tokenizer and template...")

    # Use first dataset for initial setup
    ...
    for dataset_idx, current_dataset in enumerate(dataset_names):
        if accelerator.is_main_process:
            print("\n" + "=" * 80)
            print(f"Processing dataset {dataset_idx + 1}/{len(dataset_names)}: {current_dataset}")
            print("=" * 80)

        # Get dataset file name and create output path
        dataset_file_name = dataset_info[current_dataset].get("file_name")
        ...
        # Load dataset
        if accelerator.is_main_process:
            print(f"\n[Step 3] Loading dataset: {current_dataset}...")
        ...
        if eval_dataset is None:
            raise ValueError(f"No dataset available for '{current_dataset}'. Please check your dataset configuration.")
        ...
        if accelerator.is_main_process:
            print(f"\n[Step 4] Processing data and extracting routing outputs...")
        ...
        if accelerator.is_main_process:
            print(f"\n[Step 5] Gathering results from all processes...")
        ...
        if accelerator.is_main_process:
            print(f"\n[Step 6] Saving results to {save_name}...")
            ...
            if save_dict:
                ...
                print(f"  - Shape per sample: {first_sample.shape} (num_layers, valid_seq_len, num_experts)")
    ...
    if accelerator.is_main_process:
        print("\n" + "=" * 80)
        print(f"✓ All {len(dataset_names)} datasets processed successfully!")
        print("=" * 80)
