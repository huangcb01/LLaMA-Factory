...
class GoldRouterIndicesLoader:
    r"""Loader for gold router signals for MoE auxiliary loss.

    Preferred format: per-sample arrays of FULL router logits with shape
    [num_layers, valid_seq_len, num_experts] (float32) stored in .npz files.

    Backward compatibility: If only indices arrays [num_layers, valid_seq_len, top_k]
    are available, helper methods can derive indices from logits or vice versa where possible.
    """

    def __init__(self, data_args: "DataArguments"):
        self.data_args = data_args
        self.logits_cache = {}  # Cache loaded logits files (npz)
    ...
    def load_logits_file(self, logits_path: str):
        ...
        try:
            ...
            logger.info_rank0(
                f"Loaded gold router logits from {full_path}, contains {len(logits_data.files)} samples"
            )
            return logits_data
        except Exception as e:
    ...
    def get_sample_logits(self, dataset_name: str, sample_idx: int) -> Optional[torch.Tensor]:
        """Get gold router FULL logits for a specific sample.

        Returns
        -------
        Optional[torch.Tensor]
            Tensor of shape [num_layers, seq_len, num_experts] with dtype torch.float32
        """
        ...
        # Expect float logits array: [num_layers, seq_len, num_experts]
